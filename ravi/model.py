...
class RFRNetModel:
    ...
    def initialize_model(self, path=None, train=True):
        ...
        try:
            start_iter = load_ckpt(
                path, [("generator", self.G)], [("optimizer_G", self.optm_G)]
            )
            if train:
                self.optm_G = optim.Adam(self.G.parameters(), lr=2e-4)
    ...
    def train(self, train_loader, save_path, finetune=False, iters=450000):
        ...
        if finetune:
            self.optm_G = optim.Adam(
                filter(lambda p: p.requires_grad, self.G.parameters()), lr=5e-5
            )
        ...
        while self.iter < iters:
            for items in train_loader:
                ...
                if self.iter % 50 == 0:
                    ...
                    print(
                        "Iteration:%d, l1_loss:%.4f, time_taken:%.2f"
    ...
    def get_g_loss(self):
        ...
        style_loss = self.style_loss(real_B_feats, fake_B_feats) + self.style_loss(
            real_B_feats, comp_B_feats
        )
        preceptual_loss = self.preceptual_loss(
            real_B_feats, fake_B_feats
    ...
    def style_loss(self, A_feats, B_feats):
        ...
        for i in range(len(A_feats)):
            ...
            _, c, w, h = A_feat.size()
            A_feat = A_feat.view(
                A_feat.size(0), A_feat.size(1), A_feat.size(2) * A_feat.size(3)
            )
            B_feat = B_feat.view(
                B_feat.size(0), B_feat.size(1), B_feat.size(2) * B_feat.size(3)
            )
